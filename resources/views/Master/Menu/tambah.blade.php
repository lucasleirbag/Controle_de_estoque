    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content modal-content-demo">
            <div class="modal-header">
                <h6 class="modal-title">Adicionar Menu</h6><button onclick="reset()" aria-label="Close" class="btn-close" data-bs-dismiss="modal"><span aria-hidden="true">&times;</span></button>
            </div>
            <form method="POST" action="{{ route('menu.store') }}" name="myForm" enctype="multipart/form-data" onsubmit="return validateForm()">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <span class="text-gray d-block mb-1">Pesquisar e copiar nome do ícone <a target="_blank" href="https://feathericons.com/">https://feathericons.com/</a></span></span>
                        <div class="input-group">
                            <span class="input-group-text bg-gray-light" id="basic-addon1">Cole aqui</span>
                            <input type="text" name="icon" class="form-control"  aria-label="icon" aria-describedby="basic-addon1">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="judul" class="form-label">Título</label>
                        <input type="text" id="judul" name="judul" class="form-control" placeholder="Menu">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group d-none" id="vTypeSub">
                        <div class="d-flex justify-content-end mb-2">
                            ...
                            <button type="button" onclick="addSub()" class="btn btn-primary-light">Add Submenu <i class="fa fa-plus"></i></button>
                        </div>
                ...
                <div class="modal-footer">
                    ...
                    <a href="javascript:void(0)" onclick="reset()" class="btn btn-light" data-bs-dismiss="modal">Cancelar </a>
                </div>
            </form>
        </div>
    </div>
...
<script>
    function validateForm() {
        ...
        if (icon == "") {
            validasi('Ícone obrigatório!', 'warning');
            $("input[name='icon']").addClass('is-invalid');
            return false;
        } else if (judul == '') {
            validasi('Título obrigatório!', 'warning');
            $("input[name='judul']").addClass('is-invalid');
            return false;
        } else if (type == '') {
            validasi('Tipo obrigatório!', 'warning');
            $("input[name='type']").addClass('is-invalid');
            return false;
        } else if (type == 1) {
            if (redirect == "") {
                validasi('Redirecionamento obrigatório!', 'warning');
                $("input[name='redirect']").addClass('is-invalid');
                return false;
            }
        } else if (type == 2) {
            if ($('#listsub li').length == 0){
                validasi('Não há submenu ainda!', 'warning');
                return false;
            }
        }
